...
export function useRealtimeIncidents(
  ...
  const resolveIncident = useCallback(async (id: string, resolutionType: string = 'resolved') => {
    try {
      const res = await apiFetch(`/api/incidents/${id}/resolve`, {
        method: 'POST',
  ...
  const dismissAllIncidents = useCallback(async () => {
    try {
      await apiFetch(`/api/incidents/ack-all`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const confirmIncident = useCallback(async (id: string) => {
    try {
      const res = await apiFetch(`/api/incidents/${id}/feedback`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const dispatchIncident = useCallback(async (id: string, securityId: string) => {
    try {
      const res = await apiFetch(`/api/incidents/${id}/dispatch`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
